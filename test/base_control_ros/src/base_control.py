...
class BaseControl():
    ...
    def init(self):
        ...
        self.odom_pub = rospy.Publisher("odom_fishbot",Odometry,queue_size=10)
        # 2.初始化IMU:Publisher
        self.imu_pub = rospy.Publisher("imu_fishbot",Imu,queue_size=10)
        # 3.初始化参数PID
        ...
        # 5.订阅cmd_vel
        self.cmd_vel_sub = rospy.Subscriber("cmd_vel_fishbot",Twist,self.cmd_vel_callback,queue_size=10)
        self.wheel_speed_cmd_ = [0,0] #左右轮速度
        self.wheel_seperation_ =  0.25 # 两个轮子的间距
        # start thread
    ...
    def cmd_vel_callback(self,cmd_vel_msg):
        # print(cmd_vel)
        self.last_cmd_vel_time_ = rospy.Time().now()
